"""
The module represents data from Hunspell's ``*.dic`` file. This file has the following format:
...
class Word:
    ...
    **Attributes calculated on dictionary reading:**
    ...
    #: used by ``ngram_suggest``. Not everythin specified with ``ph:`` is stored
    #: here, see :meth:`read_dic <readers.dic.read_dic>` for details.
    alt_spellings: List[str]
    #: One of ``capitalization.Type`` (no capitalization, initial letter capitalized,
    #: all letters, or mixed) analyzed on dictionary reading, will be useful on lookup.
    captype: CapType
...
class Dic:
    ...
    **Dictionary creation**
    ...
    def append(self, word: Word, *, lower: List[str]):
        """
        Used only by :meth:`read_dic <readers.dic.read_dic>` to put the word into the dictionary.
        ...
        Args:
            ...
            lower: List of all the lowercase forms of word stems. They are pre-calculated on dictionary
                   reading, because proper lowercasing requires collation; and may produce several
                   lowercased variants (for German). See :meth:`Collation.lower <algo.capitalization.Collation.lower>`
                   for details.
        """

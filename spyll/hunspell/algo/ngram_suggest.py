...
Scored = namedtuple('Scored', ['value', 'score'])

class ScoredArray:
    def __init__(self, size):
        self.data = [Scored(None, -100*i) for i in range(size)]
        self.insert_at = size - 1

    def push(self, value, score):
        if score <= self.data[self.insert_at].score: return

        self.data[self.insert_at] = Scored(value, score)
    ...
    def result(self):
        return filter(lambda s: s.value, self.data)

def ngram_suggest(dictionary, word: str, *, maxdiff, onlymaxdiff=False) -> Iterator[str]:
    ...
    guess_scores = ScoredArray(MAX_GUESSES)
    for root in root_scores.result():
        for form in dictionary.forms_for(root.value):
    ...
    guesses = sorted(guess_scores.result(), key = lambda g: -g.score)
    ...
    for item in guesses:
        ...
        score = detailed_affix_score(word, gl, fact)
        if not score:
            score = item.score + 2000
        guesses2.append(item._replace(score = score))

    guesses2 = sorted(guesses2, key = lambda g: -g.score)

    for guess in filter_guesses(guesses2, onlymaxdiff=onlymaxdiff):
        yield guess

def filter_guesses(guesses, *, onlymaxdiff=True):
    same = False
    found = 0

    for item in guesses:
        if same and item.score <= 1000: continue

        # leave only excellent suggestions, if exists
        if item.score > 1000:
            same = True
        elif item.score < -100: # FIXME: what's that? Seems related to last line of score...
            same = True
            if found > 0 and onlymaxdiff: continue
...
def first_affix_score(word1, word2):
    leftcommon = sm.leftcommonsubstring(word1, word2)
    return sm.ngram(len(word1), word1, word2.lower(), any_mismatch = True) + leftcommon

def detailed_affix_score(word1, word2, fact):
